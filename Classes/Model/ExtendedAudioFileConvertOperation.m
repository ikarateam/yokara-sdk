...
@property (nonatomic, assign) int volumeBeat;

@property (nonatomic, assign) int volumeVocal;
...
- (void) updateBeateVolume:(int)volume{
    self.volumeBeat=volume;
    
}

- (void) updateVocalVolume:(int )volume{
    self.volumeVocal=volume;
}
...
- (void)main {
    ...
    if ([self.sourceURL.path hasSuffix:@"mov"]) {
...
- (BOOL) mixAndConvert{
    ...
    // Setup buffers
    UInt32 bufferByteSize =4096* sizeof(float)*5;
    ...
    while (YES) {
        ...
        if (fillBufferList.mNumberBuffers>1) {
            ...
            if (self.effects )
                [self.effects  processFloatLeftInput:pDataL andRightInput:pDataR andLeftOutput:pDataL andRightOutput:pDataR andLength:numberOfFrames];
        }else{
            AudioBuffer *pBufferL = &fillBufferList.mBuffers[0];
            pDataL =(float *) pBufferL->mData;
            if (self.effects )
                [self.effects processFloat:pDataL andOut:pDataL andLength:numberOfFrames*fillBufferList.mBuffers[0].mNumberChannels];
        }
        ...
        for (UInt32 i = 0; i < fillBufferList.mNumberBuffers; i++)
        {
            ...
            for (int i=0; i<numberOfFrames*fillBufferList.mBuffers[0].mNumberChannels; i++) {
                ...
                pD=(pDataL[i]*(self.volumeVocal/100.0)+pDataL2[i]*(self.volumeBeat/100.0));
       ...
       // error = ExtAudioFileWrite(destinationFile, numberOfFrames, &fillBufferList);
        ...
        //write = lame_encode_buffer_ieee_float(lame,(float*) fillBufferList.mBuffers[0].mData, nil, numberOfFrames, mp3_buffer, MP3_SIZE);
    ...
    fileinfo=[[NSFileManager defaultManager] attributesOfItemAtPath:self.mp4URL.path error:nil];
    filesize=[fileinfo fileSize];
    NSLog(@"Size mp4 file : %0.2f MB",filesize/1024.0/1024.0);
    // Cleanup
    fclose(pcm);
    fclose(mp3);
    lame_close(lame);
    ...
    if (sourceFile) {
        NSLog(@"close src 1 %@",self.sourceURL);
        ExtAudioFileDispose(sourceFile);
        
    }
    if (sourceFile2) {
        ExtAudioFileDispose(sourceFile2);
