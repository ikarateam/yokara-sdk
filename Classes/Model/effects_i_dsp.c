...
{
	fft_data* fftdata = (fft_data*)calloc(1, sizeof(fft_data));
	fftdata->fft_len = -1;

  assert(fftdata->lsx_fft_br == NULL);
  assert(fftdata->lsx_fft_sc == NULL);
  assert(fftdata->fft_len == -1);
  ccrw2_init(fftdata->fft_cache_ccrw);
  fftdata->fft_len = 0;
  return fftdata;
}
...
{
  assert(fftdata->fft_len >= 0);
  ccrw2_clear(fftdata->fft_cache_ccrw);
  free(fftdata->lsx_fft_br);
  free(fftdata->lsx_fft_sc);
  fftdata->lsx_fft_sc = NULL;
  fftdata->lsx_fft_br = NULL;
  fftdata->fft_len = -1;
}

sox_bool update_fft_cache(fft_data* fftdata, int len)
{
  assert(lsx_is_power_of_2(len));
  assert(fftdata->fft_len >= 0);
  ccrw2_become_reader(fftdata->fft_cache_ccrw);
  if (len > fftdata->fft_len) {
    ccrw2_cease_reading(fftdata->fft_cache_ccrw);
    ccrw2_become_writer(fftdata->fft_cache_ccrw);
    if (len > fftdata->fft_len) {
      int old_n = fftdata->fft_len;
	  fftdata->fft_len = len;
	  fftdata->lsx_fft_br = lsx_realloc(fftdata->lsx_fft_br, dft_br_len(fftdata->fft_len) * sizeof(*(fftdata->lsx_fft_br)));
	  fftdata->lsx_fft_sc = lsx_realloc(fftdata->lsx_fft_sc, dft_sc_len(fftdata->fft_len) * sizeof(*(fftdata->lsx_fft_sc)));
      if (!old_n)
		  fftdata->lsx_fft_br[0] = 0;
      return sox_true;
    }
...
{
  sox_bool is_writer = update_fft_cache(fftdata, len);
  lsx_rdft(len, type, d, fftdata->lsx_fft_br, fftdata->lsx_fft_sc);
...
{
  sox_bool is_writer = update_fft_cache(fftdata, len);
  lsx_cdft(len, type, d, fftdata->lsx_fft_br, fftdata->lsx_fft_sc);
  done_with_fft_cache(is_writer);
}
...
void lsx_power_spectrum(fft_data* fftdata, int n, double const * in, double * out)
{
  ...
  lsx_safe_rdft(fftdata, n, 1, work);
  out[0] = sqr(work[0]);
  for (i = 2; i < n; i += 2)
...
void lsx_fir_to_phase(fft_data* fftdata, double * * h, int * len, int * post_len, double phase)
{
  ...
  lsx_safe_rdft(fftdata, work_len, 1, work); /* Cepstral: */
  ...
  lsx_safe_rdft(fftdata, work_len, -1, work);
...
void lsx_plot_fir(fft_data* fftdata, double * h, int num_points, sox_rate_t rate, sox_plot_t type, char const * title, double y1, double y2)
{
  ...
  if (type == sox_plot_gnuplot) {
    ...
    lsx_power_spectrum(fftdata, N, h1, H);
    printf(
        "# gnuplot file\n"
        "set title '%s'\n"
