...
#import "Treble.h"
#include "treble-jni.h"
#import "SoxEffects.h"
@implementation Treble
- (id) init{
    self=[super init];
    if (self) {
        Java_co_ikara_codec_Treble_nativeInit();
...
-( void )start: (int) bass { // -100 -> 100
    ...
    handler = Java_co_ikara_codec_Treble_nativeStart([bassStr UTF8String]);
    handler2 = Java_co_ikara_codec_Treble_nativeStart([bassStr UTF8String]);
}

-( void ) stop {
    Java_co_ikara_codec_Treble_nativeStop(handler);
    Java_co_ikara_codec_Treble_nativeStop(handler2);
}

-(void) updateEffect:(int) bass {
    Java_co_ikara_codec_Treble_nativeUpdateEffect(handler,[[NSString stringWithFormat:@"%d", (bass/5)] UTF8String]);
    Java_co_ikara_codec_Treble_nativeUpdateEffect(handler2, [[NSString stringWithFormat:@"%d", (bass/5)] UTF8String]);
}
...
- (void) process:(int[]) input withLength:(long)inputLength andOut: (int[]) output withLength:(long) outLength { //CHá»ˆ Xá»¬ LÃ� Vá»šI INPUT 2 KÃŠNH TRÃ�I PHáº¢I XEN Káº¼ VÃ€ LENGTH LÃ€ SoxEffects.minimumBufferSize x 2, tá»©c lÃ  cÃ³ tá»•ng cá»™ng SoxEffects.minimumBufferSize lÃ¡t cáº¯t
    ...
    Java_co_ikara_codec_Treble_nativeProcess(handler, inputLeft, outputLeft,minimumBufferSize);
    Java_co_ikara_codec_Treble_nativeProcess(handler2, inputRight, outputRight,minimumBufferSize);
...
- (int) process:(float[]) left right:( float []) right leftOut:( float []) leftOut rightOut:( float[]) rightOut andLength:(long) length{
    ...
    Java_co_ikara_codec_Treble_nativeProcess(handler, inputLeft, outputLeft,length);
    Java_co_ikara_codec_Treble_nativeProcess(handler2, inputRight, outputRight,length);
