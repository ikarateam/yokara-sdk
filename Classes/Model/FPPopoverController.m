				   ...
				   delegate:(id<FPPopoverControllerDelegate>)delegate
{
    ...
    if(self)
    {
        ...
        _touchView.backgroundColor = [UIColor colorWithRed:35/255.0 green:35/255.0 blue:35/255.0 alpha:0.8];
...
-(void)presentPopoverFromPoint:(CGPoint)fromPoint withView:(UIView*)view
{
    self.origin = fromPoint;
...
-(void)presentPopoverFromPoint:(CGPoint)fromPoint andView:(NSInteger)noView
{
    self.origin = fromPoint;
    ...
    if(windows.count > 0)
    {
	 ...
	 if(_window.subviews.count > 1)
		 {
		  _parentView = [_window.subviews objectAtIndex:_window.subviews.count-2];
		  if (![_parentView isKindOfClass:[UIButton class]]) {
              _parentView.hidden = NO;
			   [_parentView addSubview:self.view];
			   [_viewController viewDidAppear:YES];
		  }else  if(_window.subviews.count > noView-1)
			  {
			   ...
			   if (![_parentView isKindOfClass:[UIButton class]]) {
                   _parentView.hidden = NO;
					[_parentView addSubview:self.view];
					[_viewController viewDidAppear:YES];
			   }else  if(_window.subviews.count > 0)
				   {
					_parentView = [_window.subviews objectAtIndex:0];
                   _parentView.hidden = NO;
					[_parentView addSubview:self.view];
       ...
       /* if(_window.subviews.count > noView)
        {
            _parentView = [_window.subviews objectAtIndex:noView];
            if (![_parentView isKindOfClass:[UIButton class]]) {
                [_parentView addSubview:self.view];
                [_viewController viewDidAppear:YES];
            }else  if(_window.subviews.count > noView-1)
            {
                _parentView = [_window.subviews objectAtIndex:noView-1];
                if (![_parentView isKindOfClass:[UIButton class]]) {
                    [_parentView addSubview:self.view];
                    [_viewController viewDidAppear:YES];
                }else  if(_window.subviews.count > 0)
