...
static float comb_process(reverb_filter_t * p,  /* gcc -O2 will inline this */
...
static float allpass_process(reverb_filter_t * p,  /* gcc -O2 will inline this */
    float const * input)
{
  float output = *p->ptr;
...
static void filter_array_create(reverb_filter_array_t * p, double rate,
    double scale, double offset)
{
  ...
  {
	  reverb_filter_t * pcomb = &p->comb[i];
...
static void filter_array_delete(reverb_filter_array_t * p)
{
  size_t i;
...
{
	reverb_priv_t * p = (reverb_priv_t *)effp->priv;
...
sox_effect_handler_t const *lsx_reverb_effect_fn(void)
{
  static sox_effect_handler_t handler = {"reverb",
    ...
    " [wet-gain (0dB)"
    "]]]]]]",
    SOX_EFF_MCHAN, getopts, start, flow, NULL, stop, NULL, sizeof(reverb_priv_t)
  };
  return &handler;
}
