...
static void FFT(fft_data* fftdata, unsigned NumSamples,
         ...
         const float *RealIn, float *ImagIn, float *RealOut, float *ImagOut)
{
  ...
  lsx_safe_cdft(fftdata, 2 * (int)NumSamples, InverseTransform? -1 : 1, work);
...
static void reduce_noise(fft_data* fftdata, chandata_t* chan, float* window, double level)
{
    ...
    FFT(fftdata, WINDOWSIZE, 0, inr, NULL, outr, outi);

    memcpy(inr, window, WINDOWSIZE*sizeof(float));
    lsx_apply_hann_f(inr, WINDOWSIZE);
    lsx_power_spectrum_f(fftdata, WINDOWSIZE, inr, power);
    ...
    FFT(fftdata, WINDOWSIZE, 1, outr, outi, inr, ini);
    lsx_apply_hann_f(inr, WINDOWSIZE);
...
static int process_window(sox_effect_t * effp, priv_t * data, unsigned chan_num, unsigned num_chans,
    ...
    reduce_noise(effp->fftdata, chan, chan->window, data->threshold);
    if (!first) {
        for (j = 0; j < use; j ++) {
