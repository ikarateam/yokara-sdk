...
static void collect_data(fft_data* fftdata,chandata_t* chan) {
    float *out = lsx_calloc(FREQCOUNT, sizeof(float));
    int i;

    lsx_power_spectrum_f(fftdata, WINDOWSIZE, chan->window, out);
  ...
  *isamp = *osamp = n * chans;
  ...
  for (i = 0; i < chans; i ++) {
    ...
    if (n + p->bufdata == WINDOWSIZE)
      collect_data(effp->fftdata, chan);
  }
...
static int sox_noiseprof_drain(sox_effect_t * effp, sox_sample_t *obuf UNUSED, size_t *osamp)
{
    ...
    *osamp = 0;
    ...
    for (i = 0; i < tracks; i ++) {
        ...
        collect_data(effp->fftdata, &(data->chandata[i]));
    }
